...
function addDebugLine(line: string): void { debugLines.push(line); if (debugLines.length > 80) debugLines.shift(); const output = el('realtimeDebugOutput'); if (output) output.textContent = debugLines.join('\n'); }
function renderDebugPanel(): void { const root = transcriptRoot(); if (!root || el('realtimeDebugPanel')) return; const panel = document.createElement('details'); panel.id = 'realtimeDebugPanel'; panel.style.cssText = 'align-self:stretch;margin-top:8px;font-size:.75rem;color:#6B7280;'; panel.innerHTML = '<summary style="cursor:pointer;">Debug events</summary><pre id="realtimeDebugOutput" style="white-space:pre-wrap;max-height:180px;overflow:auto;background:#F3F4F6;padding:8px;border-radius:8px;margin-top:6px;"></pre><button id="realtimeDebugCopy" type="button" style="margin-top:6px;padding:6px 9px;border:1px solid #D1D5DB;border-radius:8px;background:white;">Copy log</button>'; root.appendChild(panel); el('realtimeDebugCopy')?.addEventListener('click', async () => { await navigator.clipboard.writeText(debugLines.join('\n')); const button = el('realtimeDebugCopy'); if (button) { button.textContent = 'Copied'; window.setTimeout(() => { button.textContent = 'Copy log'; }, 1200); } }); }
...
function renderEncounterIntro(): void {
  ...
  renderSupport(); renderDebugPanel();
}
...
function handleRealtimeEvent(event: RealtimeServerEvent): void {
  ...
  switch (event.type) {
    ...
    case 'conversation.item.input_audio_transcription.completed': { const text = eventText(event, 'transcript', 'text').trim(); if (text) addDebugLine(`user transcript: ${text}`); if (pendingUserMessage) { if (text) pendingUserMessage.textContent = text; else pendingUserMessage.remove(); pendingUserMessage = null; } else if (text) appendMessage('user', text); if (text) session.recordProduction(text); return; }
    case 'response.audio_transcript.done': case 'response.output_audio_transcript.done': { const text = eventText(event, 'transcript', 'text').trim(); if (text) addDebugLine(`assistant transcript: ${text}`); if (pendingTutorMessage) { if (text) pendingTutorMessage.textContent = text; else pendingTutorMessage.remove(); pendingTutorMessage = null; } else if (text) appendMessage('tutor', text); return; }
...
export async function toggleRealtimeEncounter(): Promise<void> {
  ...
  debugLines = []; const requestId = ++generationId; generating = true; setStatus('Een situatie bedenken…'); setButton('Annuleren');
  ...
  client = new RealtimeClient({ apiBase: API_BASE, word, instructions: buildTutorInstructions(encounter), onEvent: handleRealtimeEvent, onStateChange: handleStateChange, onDebug: addDebugLine });
